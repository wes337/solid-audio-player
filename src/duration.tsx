...
export default function Duration(props: DurationProps) {
  const [duration, setDuration] = createSignal<JSX.Element>(
    props.audio
      ? getDisplayTimeBySeconds(props.audio.duration, props.audio.duration, props.timeFormat)
  ...
  const handleAudioDurationChange = (event: Event): void => {
    ...
    const duration =
      getDisplayTimeBySeconds(audio.duration, audio.duration, props.timeFormat) ||
      props.defaultDuration
